...
export async function getServerSideProps({ req, locale, query }) {
  ...
  const credential = await fetch(
    `${process.env.HOSTNAME}/api/credential?userEmail=${userEmail}&token=${firebaseToken}&authorization=${process.env.NEXT_PUBLIC_API_KEY}`
  );
  ...
  const products = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/products?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const orders = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/orders?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const messages = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/messages?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const stats = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/statistics?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const coupons = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/promo?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  const couponsJSON = admin ? await coupons.json() : null;

  // Get QR to generate
  const qrToGenerate = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/qr/qrToGenerate?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const finders = admin
    ? await fetch(
        `${process.env.HOSTNAME}/api/qr/getFinders?authorization=${process.env.NEXT_PUBLIC_API_KEY}`
      )
    : null;
  ...
  const userProducts = await fetch(
    `${process.env.HOSTNAME}/api/user/products?user=${userEmail}&authorization=${process.env.NEXT_PUBLIC_API_KEY}`
  );
  const userProductsJSON = await userProducts.json();

  // Get user notifications
  const userNotifications = await fetch(
    `${process.env.HOSTNAME}/api/user/notifications?user=${userEmail}&authorization=${process.env.NEXT_PUBLIC_API_KEY}`
  );
...
export default function Dashboard(props) {
  ...
  const { user, loading, firstName, lastName, address, email } =
        ...
        <div className="flex justify-center flex-col items-center w-full ">
          ...
          <div className="text-center text-primary px-12">
            <button
              ...
              className="w-full px-4 py-4 mt-4 font-md text-white text-md bg-indigo-600 hover:bg-indigo-500 rounded-lg"
            >
          ...
          <div className="text-center text-primary px-12">
            <button
              ...
              className="w-full px-4 py-4 mt-4 font-md text-white text-md bg-emerald-600 hover:bg-emerald-500 rounded-lg"
            >
