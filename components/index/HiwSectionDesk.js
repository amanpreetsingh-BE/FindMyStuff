...
function HiwSection({ Image, useState, useEffect, motion, t, Script }) {
    ...
    const stepColor = [showStep[0] ? " text-secondary " : " text-gray-400 ", showStep[1] ? " text-secondary " : " text-gray-400 ", showStep[2] ? " text-secondary " : " text-gray-400 "]
    const stepBgOpacity = [showStep[0] ? " bg-secondary " : " bg-transparent ", showStep[1] ? " bg-secondary " : " bg-transparent ", showStep[2] ? " bg-secondary " : " bg-transparent "]
    const stepBorderOpacity = [showStep[0] ? " border-secondary " : " border-gray-400 ", showStep[1] ? " border-secondary " : " border-gray-400 ", showStep[2] ? " border-secondary" : " border-gray-400 "]
    ...
    return(
        <section id="howitworks" className="relative bg-primary w-full h-[1800px]">
            <div className="flex flex-col items-center justify-start sticky top-40 w-full h-[520px] min-h-[520px] mb-64">
                <div className="py-10 text-gray-300 font-bold text-3xl md:text-4xl">{t('home:hiw:title')}</div>
                    ...
                    <div className="text-white ml-8 lg:mx-12 relative flex items-center justify-center h-full w-1/4 max-w-xs">
                        <div className="relative flex flex-col justify-center items-center h-full w-full">
                            ...
                            <div className="relative w-2 rounded-3xl bg-gray-700 h-1/5 ml-20 mb-4">
                                <div id="progress-bar" style={showStep[0] ? {boxShadow: '0px 0px 8px 1px #7FFFD4'}:{}} className="absolute w-2 bg-secondary rounded-full h-0"></div>
                            </div>

                            <div onClick={()=>scroll.scrollTo(window.innerHeight+305)} className={"cursor-pointer w-8 h-8 md:w-10 md:h-10 border-4 border-secondary rounded-full ml-20 mb-4"+ stepBgOpacity[1] + stepBorderOpacity[1]}><div className={"absolute flex items-center justify-center w-32 -ml-32 text-secondary font-bold text-base md:text-xl "+stepColor[1]}>{t('home:hiw:lost')}</div></div>
                            
                            <div className="relative w-2 rounded-3xl bg-gray-700 h-1/5 ml-20 mb-4">
                                <div id="progress-bar2" style={showStep[1] ? {boxShadow: '0px 0px 8px 1px #7FFFD4'}:{}} className="absolute w-2 bg-secondary rounded-full h-0"></div>
                            </div>
                            
                            <div onClick={()=>scroll.scrollTo(window.innerHeight+705)} className={"cursor-pointer w-8 h-8 md:w-10 md:h-10 border-4 border-secondary rounded-full ml-20"+ stepBgOpacity[2] + stepBorderOpacity[2]}><div className={"absolute flex items-center justify-center w-32 -ml-32 text-secondary font-bold text-base md:text-xl "+stepColor[2]}>{t('home:hiw:found')}</div></div>
                        </div>
                    </div>

                    <div className={`flex items-center justify-center h-full w-1/2 max-w-xs`}>
                        <motion.div
                        ...
                        ref={ref}
                        >
                            <div className={`relative ${showStep[1] ? 'w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 lg:w-96 lg:h-96':'w-40 h-40 sm:w-48 sm:h-48 md:w-60 md:h-60 lg:w-80 lg:h-80'}  `}>
                                <Image src={illustration} priority={true} layout="fill" alt="illustration"/>
                            </div>
                        </motion.div>
                    </div>

                    <div className={`flex mr-8 lg:mx-12 text-gray-300 font-bold text-center text-sm  items-center justify-center h-full w-1/4 md:text-lg max-w-xs`}>
                        {illustrationText}
                    </div>
