...
function AdminLayout({
  ...
  const classMainSection = triggerComponent
    ? "min-h-screen"
    : "grid place-items-center max-w-7xl mx-auto px-8 py-16";
  ...
  return (
    <main className="bg-[#171C26] font-nxt text-white">
      ...
      <nav className="z-20 flex top-0 h-20">
        <div className="flex justify-between items-center pl-10 pr-10 h-full w-full ">
          ...
          <div className="flex items-center justify-center">
            <Image
              src={"/images/icons/undraw_profile_pic_ic-5-t.svg"}
              width={32}
              height={32}
              alt="profilePic"
            />
      ...
      <section className={classMainSection}>
        ...
        >
          <motion.h1
            ...
            animate={isMenu ? "open" : "closed"}
            variants={variants}
            className="tracking-wide text-xl sm:text-2xl sm:gap-4 flex justify-center items-center font-extrabold text-center "
          >
          ...
          <motion.div
            ...
            variants={variants}
            whileHover={{ scale: 1.05, transition: { duration: 0.1 } }}
            className="bg-[#64cd83] relative text-lg sm:text-xl text-center font-bold cursor-pointer flex justify-center items-center shadow-xl h-44 rounded-2xl"
          >
            <h2 className="mx-4">{t("dashboard:admin:mOrders")}</h2>
            <div
              className={
                "absolute top-2 right-2 " +
                (computeNewOrders() > 0 ? "text-red-400" : "text-white")
              }
            >
              {computeNewOrders()}
            </div>
          ...
          >
            ...
            <div
              className={
                "absolute top-2 right-2 " +
                (computeNewMessages() > 0 ? "text-red-400" : "text-white")
              }
            >
              {computeNewMessages()}
            </div>
          ...
          <motion.div
            transition={{ duration: 1 }}
            onClick={() => closeMenu("manageQR")}
            animate={isMenu ? "open" : "closed"}
            variants={variants}
            whileHover={{ scale: 1.05, transition: { duration: 0.1 } }}
            className="bg-[#64cd83] text-lg sm:text-xl text-center font-bold cursor-pointer flex justify-center items-center shadow-xl h-44 rounded-2xl col-span-2 sm:col-span-3"
          >
          ...
          <motion.h1
            ...
            animate={isMenu ? "open" : "closed"}
            variants={variants}
            className="tracking-wide text-xl sm:text-2xl sm:gap-4 flex justify-center items-center font-extrabold text-center col-span-2 my-4 sm:col-span-1 "
          >
            {t("dashboard:admin:stats")}
          </motion.h1>
          <motion.div
            transition={{ duration: 1 }}
            onClick={() => closeMenu("statsUsers")}
            animate={isMenu ? "open" : "closed"}
            variants={variants}
            whileHover={{ scale: 1.05, transition: { duration: 0.1 } }}
            className="bg-[#DBB457] text-lg sm:text-xl text-center font-bold cursor-pointer flex flex-col justify-center items-center shadow-xl hover:shadow-xl h-44 rounded-2xl"
          >
          ...
          <motion.div
            transition={{ duration: 1 }}
            onClick={() => closeMenu("statsOrders")}
            animate={isMenu ? "open" : "closed"}
            variants={variants}
            whileHover={{ scale: 1.05, transition: { duration: 0.1 } }}
            className="bg-[#DBB457] text-lg sm:text-xl text-center font-bold cursor-pointer flex flex-col justify-center items-center shadow-xl h-44 rounded-2xl col-span-1 sm:col-span-1"
          >
      ...
      {!triggerComponent ? (
        ...
        <div
          onClick={() => openMenu()}
          className="absolute cursor-pointer top-0 right-12 mt-28 flex justify-center items-center"
        >
