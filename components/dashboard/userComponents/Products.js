...
export default function Products({ useState, useRef, Modal, t, toast, Image, email, userProductsJSON }) {
  ...
  const renderProducts = (products) => {
    ...
    products.forEach(product => {
        ...
        cards.push(
            <div key={id} onClick={()=>openStockModal(id, relaisHeading, relaisCP, relaisStreet, relaisPhoto, jetons)} className="flex flex-col justify-center items-center w-52 h-52 sm:w-72 sm:h-72 rounded-lg bg-[#1c222e] shadow-lg hover:shadow-lg cursor-pointer">
                <div className="text-lg font-bold ">{id}</div>
                {jetons >= 1 ? 
                  <div className='text-emerald-500 font-bold text-sm'>{t('dashboard:user:prodPage:delivery')} {jetons}</div> :
                  <div className=' text-red-400 font-bold text-sm'>{t('dashboard:user:prodPage:delivery')} {jetons}</div>
                }
      ...
      return(
          <>
            <div className='mt-4 border-2 rounded-lg px-4 pt-4 mx-4 '>
                ...
                <button onClick={handleChangeRelais} className="max-w-lg py-4 px-8 mx-auto my-4 font-bold text-md bg-emerald-500 hover:bg-emerald-600 rounded-lg flex"><LocationMarkerIcon className='w-6 h-6 text-white' /> <span className='text-white ml-2'>{t('dashboard:user:prodPage:changeDelivery')}</span></button>
            </div>
  ...
  return (
    <div className="mt-20 mx-12 lg:mx-auto px-12 py-12 bg-[#1B212E] max-w-4xl ">
        ...
        <div className={userProductsJSON.length > 0 ? couponsClass : emptyCouponsClass}>
          {userProductsJSON.length > 0 ? renderProducts(userProductsJSON) : <div className='text-center'>{t('dashboard:user:prodPage:empty')}</div>}
          <div className='absolute'>
              <Modal showModal={showModal} setShowModal={setShowModal}>
                  <div className='w-full h-full flex flex-col justify-start items-center'>
                      <h1 className={`text-2xl font-mono ${rd || rj ? 'pt-12':'pt-24' }`}>{t('dashboard:user:prodPage:h1')}</h1>
                      {rd ? renderDelivery() : rj ? renderJetons() : <div className='mt-12 grid place-items-center gap-4 grid-cols-2'>
                        <div onClick={()=>setRd(true)} className='bg-indigo-500 cursor-pointer w-28 h-28 px-4 py-4 sm:w-40 sm:h-40 sm:px-12 sm:py-12 rounded-lg text-center flex justify-center items-center text-white font-bold'>{t('dashboard:user:prodPage:l1')}</div>
                        <div onClick={()=>setRj(true)} className='bg-emerald-500 cursor-pointer w-28 h-28 px-4 py-4 sm:w-40 sm:h-40 sm:px-12 sm:py-12 rounded-lg text-center flex justify-center items-center text-white font-bold'>{t('dashboard:user:prodPage:l2')}</div>
                      </div>}
                  </div>
              </Modal>
          </div>
