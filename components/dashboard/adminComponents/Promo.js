export default function Promo({
    ...
    coupons.forEach((coupon) => {
      ...
      cards.push(
        <div
          key={promoID}
          onClick={() =>
            openStockModal(promoID, couponID, codeName, off, usage)
          }
  ...
  return (
    <div className="mt-20 mx-12 lg:mx-auto px-1 py-12 max-w-4xl ">
      ...
      <div
        onClick={() => setIsCouponsMenu(!isCouponsMenu)}
      ...
      {isCouponsMenu ? (
        <div
          className={couponsJSON.length > 0 ? couponsClass : emptyCouponsClass}
        >
          {couponsJSON.length > 0 ? (
            renderProducts(couponsJSON)
          ) : (
            <div>No Items</div>
          )}
            ...
            <Modal showModal={showModal} setShowModal={setShowModal}>
              <div className="w-full h-full flex flex-col justify-evenly items-center">
                ...
                <ul className="list-disc font-medium">
                  <li>Promo ID : {modalPromoID}</li>
                  <li>Coupon ID : {modalCouponID}</li>
                  <li>Code name : {modalCodeName}</li>
                  <li>Percentage off : {modalOFF + "%"}</li>
                  <li>Used : {modalUsage}</li>
                </ul>
                <button
                  onClick={handleDel}
      ...
      <div
        ...
        className="flex mb-8 bg-emerald-600 hover:bg-emerald-500 cursor-pointer transition duration-150 ease-in-out rounded-lg mx-12 px-8 py-8 justify-center max-w-7xl xl:mx-auto items-center font-bold text-xl shadow-lg"
      >
        Add coupons
      </div>
      {isAddMenu ? (
        <div>
          <div className="mx-auto px-20 border-white ">
            ...
            <div className="flex justify-center items-center">
              <button
                onClick={handleAdd}
                className="px-3 mt-12 py-4 font-medium bg-blue-500 hover:bg-blue-600 rounded-lg"
              >
                Create a new coupon
              </button>
            </div>
