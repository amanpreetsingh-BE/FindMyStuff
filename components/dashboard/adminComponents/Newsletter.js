...
function Newsletter({ useState, useRef, toast, hostname }) {
  ...
  return (
      <div className="mt-20 mx-12 lg:mx-auto px-12 py-12 bg-[#1B212E] max-w-4xl ">
          ...
          <label htmlFor="msg" className="block mt-12 text-sm font-medium">Text (HTML)</label>
          <div className="mt-1">
            <textarea className="resize-none" id="msg" name="msg" cols="10" rows="10" minLength="10" ref={message} required/>
          </div>
          {file ? 
          <>
            {file.name}
            <div className="flex justify-end items-center w-full mb-4 text-sm">
                <div className='flex justify-center cursor-pointer items-center' onClick={handleReset}><BackspaceIcon className='w-7 h-7 mr-1'/> Reset</div>
            </div>
          </>
          : 
          <>
            ...
            <label htmlFor="file" className="font-medium cursor-pointer text-white bg-blue-500 hover:bg-blue-600 px-8 py-4 mt-8 flex justify-center items-center"> 
                <CloudUploadIcon className="w-8 h-8 mr-1 " />Upload file
            </label>
          </>}
          <div className="flex justify-center items-center">
              <button onClick={handleSubmit} className="px-24 mt-12 py-4 font-medium bg-emerald-500 hover:bg-emerald-600 rounded-lg">Send a newsletter</button>
          </div>
      </div>
  )
}
