function ManageOrders({ useState, hostname, Modal, ordersJSON, toast, t  }) {
    ...
    function openOrderModal(email, name, city, line1, line2, postal, emailSent, cs, shipped){
    ...
    ordersJSON.forEach(element => {
        ...
        const emailSent = element.emailSent ? 'Confirmation email sent' : 'Error sending confirmation email !'
        const bgColor = element.shipped ? 'bg-emerald-600' :'bg-red-500'
        tr.push(
            <tr key={element.stripe_checkoutID} onClick={()=>openOrderModal(email, name, city, line1, line2, postal, emailSent, cs, shipped)} className={'cursor-pointer border-b-2 border-b-[#1B212E] text-white font-medium last-of-type:border-b-emerald-600   ' + bgColor}>
                ...
                <Modal showModal={showModalOrder} setShowModal={setShowModalOrder}>
                    <div className='w-full h-full flex flex-col justify-evenly items-center'>
                        ...
                        <ul className="list-disc font-medium">
                            <li>
                                Customer email : {modalOrderCustomerEmail}
                            </li>
                            <li>
                                Customer full name : {modalOrderShippingName}
                            </li>
                            <li>
                                Line 1 : {modalOrderLine1}
                            </li>
                            <li>
                                Line 2 : {modalOrderLine2}
                            </li>
                            <li>
                                City : {modalOrderCity}
                            </li>
                            <li>
                                Postal code : {modalOrderPostalCode}
                            </li>
                            <li>
                                Confirmation email sent : {modalOrderEmailSent}
                            </li>
                            <li>
                                Shipped  : {`${modalOrderShipped} `}
                            </li>
                        </ul>
                        <button disabled={loading} onClick={updateOrder} className='px-8 py-4 bg-indigo-600 hover:bg-indigo-500 rounded-md text-white'>
