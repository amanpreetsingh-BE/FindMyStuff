...
function MobileNav({ Image, Link, toggle, isConnected, t, isOpen }) {
  ...
  return (
    <aside
      className={
        "z-40 font-nxt fixed bg-menuBg w-screen h-full grid items-center transition ease-in-out duration-300 left-0 text-white " +
        opacity +
        top
      }
    >
      <div
        onClick={toggle}
        className="absolute top-8 right-8 w-8 h-8 cursor-pointer outline-none"
      >
